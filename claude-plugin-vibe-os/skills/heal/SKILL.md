...
- Include `Co-Authored-By: VibeOS CI Healer <noreply@vibeos.dev>` trailer
...
```bash
git tag -l "vibeos-checkpoint-*" | sort -r | head -1
```
...
Write a CI heal report to `.vibeos/` for historical tracking and Performance Coach analysis.

### Create Reports Directory

```bash
mkdir -p .vibeos/ci-heals
```
...
Write the JSON report to `.vibeos/ci-heals/heal-${HEAL_TIMESTAMP}.json` using the temp-file-then-mv pattern:

```bash
cat > ".vibeos/ci-heals/heal-${HEAL_TIMESTAMP}.json.tmp" << 'REPORT_EOF'
{ ... report JSON ... }
REPORT_EOF
mv ".vibeos/ci-heals/heal-${HEAL_TIMESTAMP}.json.tmp" ".vibeos/ci-heals/heal-${HEAL_TIMESTAMP}.json"
```

### Display Report Location

```
Report saved: .vibeos/ci-heals/heal-<timestamp>.json
```
