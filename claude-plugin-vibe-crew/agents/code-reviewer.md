...
### Step 10.5: UI Gotchas Compliance

Read `${CLAUDE_PLUGIN_ROOT}/templates/ui-gotchas.md`. For each rule applicable to the changed files, verify compliance. Report findings using these severity defaults:

| Category | Critical | Warning |
|----------|----------|---------|
| Forms | Placeholder-only labels | Validation timing, missing autocomplete, missing aria-invalid |
| Media | Animated GIFs, `font-display: block`, lazy-loaded viewport images | Missing srcset, missing font metric overrides, missing dimensions |
| Mobile | `user-scalable=no` or `maximum-scale=1` | `100vh` instead of `100dvh`, sub-16px input font-size, missing safe-area |
| Animation | — | Non-GPU properties animated, missing reduced-motion, >600ms duration |
| Auth | Account enumeration (specific login errors) | Confirm-password field, missing recovery flow |
| Dark patterns | Consent asymmetry, pre-checked options | Confirmshaming, fake urgency, hidden costs, forced action, nagging, sneaking |
| Legal | Missing privacy policy, ungated third-party scripts | Missing impressum, missing cookie consent |
| SEO | Missing title or meta description | Missing OG tags, missing structured data |
| Social | Auto-loading social platform scripts | Missing rel="noopener" |
| Settings | Displayed full API keys, one-click destructive actions | Missing save feedback |
| Notifications | — | Auto-dismissed error toasts, focus-stealing toasts |
| Onboarding | Forced tours (no skip), missing focus management | localStorage-only persistence |
| Collaboration | Client-only permission checks, per-character aria-live | Silent rollbacks, missing reconnection strategy |

Category for all findings: `"ui-gotchas-compliance"`.
  ...
  "findings": [
    {
      ...
      "category": "correctness|tdr-compliance|architecture-consistency|convention|code-quality|design-system|visual-compliance|error-handling|test-coverage|security|performance|ui-gotchas-compliance|legal-compliance",
