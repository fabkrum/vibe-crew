...
# --- Find project root by walking up from script location looking for .vibecrew/ ---
find_project_root() {
  local dir
  dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  while [[ "$dir" != "/" ]]; do
    if [[ -d "$dir/.vibecrew" ]]; then
      echo "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
...
PROJECT_ROOT="$(find_project_root)" || {
  # No .vibecrew/ found — not inside a VibeCrew project, nothing to do
  exit 0
}
