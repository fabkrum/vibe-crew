...
# Output: .vibeos/handoffs/handoff-{YYYY-MM-DD}-{NNN}.md
# Exit 0 always.
...
HANDOFFS_DIR="$PROJECT_ROOT/.vibeos/handoffs"
...
SCORE="N/A"
RATING="N/A"
LATEST_SCORE=$(ls -1t "$PROJECT_ROOT/.vibeos/scores"/score-*.json 2>/dev/null | head -1)
...
SIGNALS=$(ls "$PROJECT_ROOT/.vibeos/signals"/*.signal 2>/dev/null || echo "")
if [[ -n "$SIGNALS" ]]; then
