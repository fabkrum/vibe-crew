...
# VibeOS Handoff: Cross-Session Context Transfer

You are the VibeOS handoff generator. Your job is to create a concise, structured context document that transfers knowledge from the current session to the next. The handoff document enables seamless session continuation without losing context. Keep it under 500 words.
...
}' .vibeos/backlog.json 2>/dev/null || echo "{}"
```
...
```bash
ls -1t .vibeos/sessions/*.json 2>/dev/null | head -1 | xargs cat 2>/dev/null || echo "{}"
```

### 1.5 Read latest Vibe Score

```bash
ls -1t .vibeos/scores/*.json 2>/dev/null | head -1 | xargs jq '{score, rating, deductions: [.deductions[].category]}' 2>/dev/null || echo "{}"
```
...
- Unresolved signals in `.vibeos/signals/`
- Stale locks

```bash
ls .vibeos/signals/*.signal 2>/dev/null || echo "no signals"
```
...
Create the handoff document using the template structure. Write to `.vibeos/handoffs/handoff-{YYYY-MM-DD}-{NNN}.md`.

```bash
mkdir -p .vibeos/handoffs
today=$(date -u +%Y-%m-%d)
existing=$(ls .vibeos/handoffs/handoff-${today}-*.md 2>/dev/null | wc -l | tr -d ' ')
next=$(printf "%03d" $((existing + 1)))
handoff_file=".vibeos/handoffs/handoff-${today}-${next}.md"
```
