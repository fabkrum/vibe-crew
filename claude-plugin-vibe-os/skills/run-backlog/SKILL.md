...
```bash
jq -r '.foundation.complete' .vibeos/state.json 2>/dev/null || echo "error"
```
...
```bash
jq '[.features[] | select(.column == "ready")] | length' .vibeos/backlog.json 2>/dev/null || echo "0"
```
...
```bash
cat .vibeos/config.json 2>/dev/null || echo '{"error": "no config"}'
```
...
```bash
jq -r '.active_feature.id // "none"' .vibeos/state.json 2>/dev/null || echo "none"
```
...
```bash
jq -r '[.features[] | select(.column == "ready")] | sort_by(.priority) | .[] | "\(.priority)\t\(.id)\t\(.name)\t\(.dependencies // [] | join(","))"' .vibeos/backlog.json
```
...
jq --arg id "<feature-id>" --arg name "<feature-name>" --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  ...
  .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
```bash
jq --arg id "<feature-id>" '.features[] | select(.id == $id) | .spec' .vibeos/backlog.json
```
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/notify.sh" "VibeOS" "Feature blocked: {name} — tests failed after 3 retries"
```
...
3. **Release notes data**: Write a release notes entry to `.vibeos/releases/`:

```bash
mkdir -p .vibeos/releases
```

Write a JSON file to `.vibeos/releases/<feature-id>.json`:
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/notify.sh" "VibeOS" "Quality gate FAILED: {name} — blocked after 3 retries"
```
...
Write to `.vibeos/scores/<feature-id>.json`:
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/notify.sh" "VibeOS" "Backlog run complete: {completed}/{total} features processed"
```

### Log the session

Write a session log to `.vibeos/sessions/`:

```bash
mkdir -p .vibeos/sessions
```

Write to `.vibeos/sessions/backlog-run-<timestamp>.json`:
...
- **Do NOT modify files outside the project directory.** All writes go to the project tree, `.vibeos/`, or `docs/`.
