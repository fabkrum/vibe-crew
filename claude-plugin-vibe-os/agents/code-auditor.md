...
Write the findings to `.vibeos/onboard-findings.json` using the temp file pattern:

```bash
cat > .vibeos/onboard-findings.json.tmp << 'FINDINGS_EOF'
{ ... findings JSON ... }
FINDINGS_EOF
mv .vibeos/onboard-findings.json.tmp .vibeos/onboard-findings.json
```
...
- **Existing .vibeos/**: If `.vibeos/` already exists, note it in findings. The `/onboard` command will ask the user whether to re-onboard.
