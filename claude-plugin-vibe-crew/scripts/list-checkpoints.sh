...
# Lists all VibeCrew checkpoint tags and recent git commits.
...
# --- Collect VibeCrew checkpoint tags ---
CHECKPOINTS="[]"
...
done < <(git tag -l 'vibecrew-checkpoint-*' --sort=-creatordate 2>/dev/null)
