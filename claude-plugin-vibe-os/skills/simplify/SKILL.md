...
Verify that VibeOS is initialized and an active feature exists:
...
If `.vibeos/state.json` exists, read it and check for an active feature:

```bash
jq -r '.active_feature.id // empty' .vibeos/state.json
```
...
```bash
FEATURE_ID=$(jq -r '.active_feature.id' .vibeos/state.json)
FEATURE_NAME=$(jq -r '.active_feature.name' .vibeos/state.json)
...
```bash
mkdir -p .vibeos/simplifications
```
...
3. Produce a structured simplification report JSON written to `.vibeos/simplifications/simplify-<feature_id>-<timestamp>.json`

Wait for the agent to complete and read the report file:

```bash
ls -t .vibeos/simplifications/simplify-*.json 2>/dev/null | head -1
```
...
Net lines changed: <+/-N>
Report: .vibeos/simplifications/simplify-<feature_id>-<timestamp>.json
```
...
```bash
REPORT_FILE=$(ls -t .vibeos/simplifications/simplify-*.json 2>/dev/null | head -1)
...
- Save the full simplification report to `.vibeos/simplifications/` before exiting.
