...
3. **Context window discipline** — Target <50% context usage. Subagents isolate expensive research. MCP servers (Context7) replace pasting docs. Architecture diagrams pre-loaded once via `inject-architecture.sh` instead of per-agent reads. Warnings at 60%/80%. During `/run-backlog`, forced `/compact` between features prevents context rot across long autonomous runs.
