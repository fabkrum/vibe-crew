...
# VibeOS Wrap: End-of-Session Wrap-Up

You are the VibeOS Verifier agent running the end-of-session wrap-up sequence. Your job is to run the quality gate, calculate the Vibe Score, write the session log and score file, commit changes, and optionally create a pull request. Execute each step sequentially. Do NOT skip steps or reorder them.

---

## Step 1: Pre-flight

### 1.1 Verify VibeOS is initialized
...
```bash
cat .vibeos/backlog.json 2>/dev/null || echo '{"features": []}'
```
...
```bash
ls -1t .vibeos/sessions/ 2>/dev/null | head -1
```
...
```bash
jq -r '.context_warnings_triggered // empty' .vibeos/state.json 2>/dev/null
```
...
```bash
jq -r '.active_feature.phases_completed | join(",")' .vibeos/state.json 2>/dev/null || echo "none"
```
...
```bash
jq --arg id "<feature_id>" '.features[] | select(.id == $id) | .spec.acceptance_criteria | length' .vibeos/backlog.json 2>/dev/null || echo "0"
```
...
```bash
today=$(date -u +%Y-%m-%d)
existing=$(ls .vibeos/scores/score-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
...
```bash
mkdir -p .vibeos/scores
```

Write the score file to `.vibeos/scores/score-<YYYY-MM-DD>-<NNN>.json` using the following schema. Use `jq` or direct file writing with proper JSON formatting:
...
cat > .vibeos/scores/score-${today}-${next}.json.tmp << 'SCORE_EOF'
{ ... score JSON ... }
SCORE_EOF
mv .vibeos/scores/score-${today}-${next}.json.tmp .vibeos/scores/score-${today}-${next}.json
```
...
Write to `.vibeos/sessions/session-<YYYY-MM-DD>-<NNN>.json` with this schema:
...
```bash
cat > .vibeos/sessions/<filename>.tmp << 'SESSION_EOF'
{ ... session JSON ... }
SESSION_EOF
mv .vibeos/sessions/<filename>.tmp .vibeos/sessions/<filename>
```
...
```bash
jq '.active_feature = {id: null, name: null, worktree: null, phase: null, phases_completed: []} | .updated_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```

If the feature is still in progress (not all phases done), leave `active_feature` as-is so the next session can resume:

```bash
jq '.updated_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
jq --arg id "<feature_id>" --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
   ...
   .vibeos/backlog.json > .vibeos/backlog.json.tmp && mv .vibeos/backlog.json.tmp .vibeos/backlog.json
```
