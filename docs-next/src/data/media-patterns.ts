...
export const mediaPatterns: MediaPattern[] = [
  ...
  },
  {
    ...
    category: 'optimization',
    description: 'Choosing and converting to modern image formats — AVIF (best compression, slow encode), WebP (good compression, wide support), JPEG (universal fallback), PNG (lossless, transparency). Serve modern formats with <picture> fallbacks. A single image pipeline: source → AVIF + WebP + JPEG at multiple sizes. AVIF saves ~50% vs JPEG, WebP saves ~30%. SVG for icons and illustrations (infinitely scalable, tiny file size). CRITICAL: never use animated GIFs — they are massive (10-20x larger) and lack playback controls. For any animated/looping visual content, use a silent autolooping video instead: <video autoplay muted loop playsinline>. An MP4/WebM video at equivalent quality is 80-90% smaller than the GIF, supports hardware decoding, and allows the browser to pause off-screen loops automatically.',
