...
# Common error recovery utility for VibeOS.
...
# Remove .vibeos/locks/*/ directories older than 60 seconds
# =============================================================================

cleanup_stale_locks() {
  local locks_dir="$VIBEOS_DIR/locks"
  if [[ ! -d "$locks_dir" ]]; then
    return 0
  fi
...
# =============================================================================
# 5. Temp file cleanup
# Remove .vibeos/*.tmp files older than 5 minutes
# =============================================================================
...
cleanup_temp_files() {
  ...
  for tmp_file in "$VIBEOS_DIR"/*.tmp "$VIBEOS_DIR"/*.tmp.*; do
