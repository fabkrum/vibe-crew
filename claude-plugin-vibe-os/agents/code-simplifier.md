...
Write the complete findings as a JSON report to `.vibeos/simplifications/`.

Generate the filename:

```bash
FEATURE_ID=$(jq -r '.active_feature.id' .vibeos/state.json 2>/dev/null || echo "unknown")
TIMESTAMP=$(date -u +"%Y%m%dT%H%M%SZ")
REPORT_PATH=".vibeos/simplifications/simplify-${FEATURE_ID}-${TIMESTAMP}.json"
```
...
- Only use Bash for read-only commands: `cat`, `ls`, `find`, `grep`, `wc`, `jq`, `git log`, `git status`, `git diff`, and writing the report to `.vibeos/simplifications/` via the temp-file-then-mv pattern.
- The ONLY file you are allowed to create is the simplification report in `.vibeos/simplifications/`.
