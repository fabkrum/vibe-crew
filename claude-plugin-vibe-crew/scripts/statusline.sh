...
# Reads session JSON from stdin, reads .vibecrew/ state files from disk.
...
# --- Disk reads: .vibecrew/ state files ---
...
if [[ -f "$VIBECREW_DIR/state.json" ]]; then
  STATE_JSON="$(jq -r '.' "$VIBECREW_DIR/state.json" 2>/dev/null || echo '{}')"
...
SCORE=""
TREND=""
LATEST_SCORE="$(ls -t "$VIBECREW_DIR/scores"/score-*.json 2>/dev/null | head -1 || echo '')"
...
COMPACTIONS="0"
LATEST_SESSION="$(ls -t "$VIBECREW_DIR/sessions"/session-*.json 2>/dev/null | head -1 || echo '')"
