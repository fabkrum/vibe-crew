...
4.5. Read `${CLAUDE_PLUGIN_ROOT}/templates/ui-gotchas.md` for non-obvious UI rules. Cross-reference each gotcha against the feature spec and note applicable rules in the relevant design spec sections (e.g., form gotchas → "Form Patterns Applied", animation gotchas → "Animation Patterns Applied"). If `spec.expected_action` exists, ensure the primary CTA is visually dominant (primary Button variant, above the fold).
