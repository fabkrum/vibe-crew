...
- **Use specific error types.** Throw `new OrderNotFoundError(orderId)` not `new Error("something went wrong")`. Callers should be able to distinguish error types.
...
- **Always `await` promises** — never fire-and-forget. If you intentionally don't await, add `void` prefix and a comment explaining why: `void logAnalytics(event); // fire-and-forget, failure is non-critical`.
...
- **No barrel re-exports that pull in everything.** `export * from './components'` causes bundle bloat. Export specific items.
...
| 9 | Nesting deeper than 2 levels | `warning` |
...
| 17 | Generic error messages | `warning` |
...
| 28 | Barrel re-exports (`export * from`) | `warning` |
