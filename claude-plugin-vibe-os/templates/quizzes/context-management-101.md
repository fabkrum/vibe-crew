...
What is the primary purpose of the Context7 MCP server in VibeOS?
...
**Explanation:** Context7 is a Model Context Protocol server that fetches library documentation on demand, eliminating the need to paste large doc blocks directly into the chat. This is a core context discipline strategy in VibeOS — MCP servers replace manual doc pasting, keeping the context window lean and reducing token waste.

---

## Q2

At what context usage percentage does VibeOS first warn you to consider wrapping the session?
...
**Explanation:** The `check-context.sh` hook fires warnings at both 60% and 80% context usage. The 60% warning is your first signal that it may be time to plan a clean handoff. VibeOS targets keeping active context below 50% as its ideal operating condition, so 60% is the early alert threshold.

---

## Q3

Why does VibeOS route Stack Scout research into an isolated subagent context rather than running it in the main session?
...
**Explanation:** Research tasks — fetching docs, scanning libraries, comparing options — are context-expensive. By isolating Stack Scout in its own context window, VibeOS keeps the main development session clean and cache-friendly. The subagent produces a compact TDR artifact as its output, not a wall of raw research text, so only the decision record enters the main session.
...
**Explanation:** Low cache utilization receives a -15 deduction from the Vibe Score. Cache utilization measures how effectively you are reusing previously computed token outputs rather than regenerating them. High cache hit rates reduce cost and latency, so VibeOS treats cache discipline as a first-class performance metric.

---

## Q5

A developer has been working in the same Claude Code session for several hours and is deep into implementing a complex feature. The context window is at 78%. What is the VibeOS-recommended action?
