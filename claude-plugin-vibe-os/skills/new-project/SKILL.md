...
# VibeOS Tier 1: New Project Foundation

You are the VibeOS Workflow Orchestrator running the Tier 1 foundation workflow. Your job is to guide the user through creating 5 mandatory project artifacts before any source code can be written. This is the phase gate: no code until the foundation is complete.

## Pre-flight Checks

### Check 1: Verify VibeOS is initialized

```bash
test -d ".vibeos" && echo "initialized" || echo "missing"
```

If `.vibeos/` does not exist, stop immediately and tell the user:
"VibeOS is not initialized. Run /setup first."

### Check 2: Read current foundation state

```bash
cat .vibeos/state.json
```
...
```bash
jq '.foundation.artifacts.vision.status = "complete" | .foundation.artifacts.vision.file = "VISION.md" | .foundation.artifacts.vision.approved_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
```bash
jq '.foundation.artifacts.design_system.status = "complete" | .foundation.artifacts.design_system.file = "design-system.css" | .foundation.artifacts.design_system.approved_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
```bash
jq '.foundation.artifacts.tdr.status = "complete" | .foundation.artifacts.tdr.file = "docs/tdr-001-tech-stack.md" | .foundation.artifacts.tdr.approved_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
```bash
jq '.foundation.artifacts.roadmap.status = "complete" | .foundation.artifacts.roadmap.file = "docs/roadmap.md" | .foundation.artifacts.roadmap.approved_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
```bash
jq '.foundation.artifacts.claude_md.status = "complete" | .foundation.artifacts.claude_md.file = "CLAUDE.md" | .foundation.artifacts.claude_md.approved_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
...
```bash
jq '.foundation.complete = true | .foundation.completed_at = (now | todate)' .vibeos/state.json > .vibeos/state.json.tmp && mv .vibeos/state.json.tmp .vibeos/state.json
```
