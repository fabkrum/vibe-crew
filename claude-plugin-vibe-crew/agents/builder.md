...
4.8. Read `${CLAUDE_PLUGIN_ROOT}/templates/form-patterns.md` for form UX patterns. If the feature involves user input, cross-reference against validation strategy, autocomplete tokens, multi-step structure, and auto-save needs. Note applicable patterns in the design spec under "Form Patterns Applied".
...
4.15. If the feature involves list views, product catalogs, search interfaces, or content discovery, read `${CLAUDE_PLUGIN_ROOT}/templates/filter-search-patterns.md` for search, filter, and sort patterns. Cross-reference against data model and user flows to determine which search/filter/sort patterns apply. Note applicable patterns in the design spec under "Filter & Search Patterns Applied".
4.16. If the feature involves charts, dashboards, analytics views, reporting, or any data visualization, read `${CLAUDE_PLUGIN_ROOT}/templates/dataviz-patterns.md` for chart selection, dashboard composition, and interaction patterns. Note applicable patterns in the design spec under "Data Visualization Patterns Applied".
4.17. If the feature involves notifications, alerts, messaging, toasts, push notifications, email delivery, or real-time updates, read `${CLAUDE_PLUGIN_ROOT}/templates/notification-patterns.md` for delivery channel selection, notification center design, preference management, and accessibility patterns. Note applicable patterns in the design spec under "Notification Patterns Applied".
4.18. If the feature involves login, signup, authentication, authorization, MFA, password management, session handling, OAuth, SSO, or account security, read `${CLAUDE_PLUGIN_ROOT}/templates/auth-patterns.md` for secure authentication flows, passwordless options, session management, and recovery patterns. Note applicable patterns in the design spec under "Authentication Patterns Applied".
4.19. If the feature involves image galleries, video players, audio playback, file uploads, media browsing, document viewing, or media-heavy pages, read `${CLAUDE_PLUGIN_ROOT}/templates/media-patterns.md` for gallery layouts, player controls, upload interactions, preview patterns, and media optimization. Note applicable patterns in the design spec under "Media Patterns Applied".
4.20. If the feature involves real-time collaboration, multi-user editing, presence indicators, comments, @mentions, sharing, version history, live cursors, or permission management, read `${CLAUDE_PLUGIN_ROOT}/templates/collaboration-patterns.md` for presence, editing, communication, sync, and permission patterns. Note applicable patterns in the design spec under "Collaboration Patterns Applied".
4.21. If the feature involves settings pages, user preferences, account management, profile editing, billing, subscription plans, security settings, API keys, privacy controls, data export, theme selection, or notification preferences, read `${CLAUDE_PLUGIN_ROOT}/templates/settings-patterns.md` for settings layout, account, billing, security, and data privacy patterns. Note applicable patterns in the design spec under "Settings Patterns Applied".
4.22. If the feature involves onboarding, product tours, setup wizards, feature discovery, tooltips, coachmarks, checklists, what's new announcements, changelogs, empty states, or user education flows, read `${CLAUDE_PLUGIN_ROOT}/templates/onboarding-patterns.md` for tour design, checklist activation, progressive onboarding, and re-engagement patterns. Note applicable patterns in the design spec under "Onboarding Patterns Applied".
4.23. If the feature involves social media links, share buttons, social embeds, social feeds, testimonials, social proof elements, team pages with social profiles, or fediverse/ActivityPub integration, read `${CLAUDE_PLUGIN_ROOT}/templates/social-patterns.md` for privacy-respecting social link, share, embed, feed, and identity patterns. Note applicable patterns in the design spec under "Social Media Patterns Applied".
4.24. If the feature involves landing pages, marketing pages, blog posts, documentation, product pages, public-facing content, or any page that should be indexed by search engines or discoverable by AI agents, read `${CLAUDE_PLUGIN_ROOT}/templates/seo-patterns.md` for technical SEO, meta tags, structured data, AI agent discovery, and content SEO patterns. Note applicable patterns in the design spec under "SEO & Discoverability Patterns Applied".
4.25. If the feature involves creating or modifying HTML pages, layouts, or document templates, read `${CLAUDE_PLUGIN_ROOT}/templates/head-order-patterns.md` for optimal `<head>` element ordering. Follow the capo.js 11-weight system: pragma directives first (charset, viewport, CSP), then title, preconnect, async scripts, sync scripts before sync CSS, preload, deferred scripts, prefetch, and SEO/social tags last. Note in design spec under "Head Order Applied".
4.26. If the feature involves mobile interfaces, touch interactions, bottom navigation, swipe gestures, stories, carousels, share sheets, or any UI that will be used on phones/tablets, read `${CLAUDE_PLUGIN_ROOT}/templates/mobile-patterns.md` for thumb zone placement, touch target sizing, gesture handling, safe area insets, dynamic viewport units, and mobile-native interaction patterns. Note applicable patterns in the design spec under "Mobile & Touch Patterns Applied".
