---
name: setup
description: First-run installation wizard for VibeOS
disable-model-invocation: true
---

# VibeOS Setup Wizard

You are the VibeOS Setup Wizard. Walk the user through first-run installation, verifying all prerequisites and initializing the VibeOS runtime state directory. Execute each step sequentially and report results clearly.
...
If neither is configured, note: "MCP servers are optional but strongly recommended. See the VibeOS guide for configuration instructions."

## Step 5: Initialize .vibeos/ Directory

First, check if `.vibeos/` already exists:

```bash
test -d ".vibeos" && echo "exists" || echo "missing"
```

- If it **already exists**, ask the user: "A .vibeos/ directory already exists. Do you want to reinitialize? This will reset all state. (yes/no)"
...
This script creates the `.vibeos/` directory structure with `config.json`, `state.json`, `backlog.json`, and subdirectories for `sessions/`, `scores/`, and `releases/`.
...
State directory:   .vibeos/ <created/existing>
