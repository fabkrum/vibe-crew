...
# =============================================================================
# 1. No-op when files are already at current version (1.5.0)
# =============================================================================

@test "no-op when all files are already at current version 1.5.0" {
  # Default fixtures from common-setup are at 1.4.0; write 1.5.0 versions
  write_versioned_state "1.5.0"
  write_versioned_config "1.5.0"
  write_versioned_backlog "1.5.0"
...
# =============================================================================
# 2. Migrates state.json from 1.0.0 to 1.5.0
# =============================================================================

@test "migrates state.json from 1.0.0 to 1.5.0" {
  ...
  run_migrate
  assert_success
  assert_output --partial "Migrated state.json from 1.0.0 to 1.5.0"
...
@test "migrates config.json from 1.0.0 adding all new sections" {
  ...
  run_migrate
  assert_success
  assert_output --partial "Migrated config.json from 1.0.0 to 1.5.0"
  ...
  # 1.1.0 -> 1.2.0: audit
  [ "$(jq -r '.audit.auto_github_issues' "$VIBECREW_DIR/config.json")" = "false" ]
...
@test "forward-compatibility: skips files with newer schema_version" {
  ...
  run_migrate
  assert_success
  assert_output --partial "newer than 1.5.0"
...
@test "migrates score files from 1.0.0" {
  ...
  # Verify schema_version updated
  [ "$(jq -r '.schema_version' "$score_file")" = "1.5.0" ]
}
...
@test "updates schema_version to 1.5.0 after migration on all files" {
  write_versioned_state "1.0.0"
  write_versioned_config "1.0.0"
  write_versioned_backlog "1.0.0"
  ...
  # All three files should be at 1.5.0
  [ "$(jq -r '.schema_version' "$VIBECREW_DIR/state.json")" = "1.5.0" ]
  [ "$(jq -r '.schema_version' "$VIBECREW_DIR/config.json")" = "1.5.0" ]
  [ "$(jq -r '.schema_version' "$VIBECREW_DIR/backlog.json")" = "1.5.0" ]
}
