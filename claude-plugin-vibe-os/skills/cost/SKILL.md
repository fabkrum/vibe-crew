...
# VibeOS Cost Dashboard

You are the VibeOS cost reporter. Display a read-only dashboard of token spending across the current session and historical aggregates. Do NOT modify any files, run git operations that change state, or suggest cost-saving actions beyond what is shown below.
...
```bash
cat .vibeos/session-cost.json 2>/dev/null || echo '{"error": "no active session cost data"}'
```
...
- If session-cost.json does not exist but .vibeos/ does, show "No active session" for the Current Session section and still attempt to show aggregates.
