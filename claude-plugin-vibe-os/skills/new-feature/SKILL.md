...
1. Check whether the `.vibeos/` directory exists in the project root.
   - If it does **not** exist, stop and tell the user:
     > VibeOS is not initialized in this project. Run `/setup` first.
   - Do NOT proceed.
2. Read `.vibeos/state.json`.
...
1. Read `.vibeos/backlog.json`.
2. Count all features in the `features` array whose `column` value is `"in-progress"`.
3. The default WIP limit is **1**. If a custom limit is set in `.vibeos/config.json` at `wip_limit`, use that value instead.
...
1. Read `.vibeos/backlog.json` and iterate over the `features` array.
...
2. Update `.vibeos/state.json` with the active feature details:
   ```json
   {
     "active_feature": {
       "id": "<feature ID>",
