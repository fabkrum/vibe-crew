...
# =============================================================================
# uninstall.sh — Clean removal of VibeOS plugin files
# =============================================================================
# Removes the claude-plugin-vibe-os directory and optionally the .vibeos/
...
info()  { printf "${BOLD}[VibeOS]${NC} %s\n" "$1"; }
warn()  { printf "${YELLOW}[VibeOS]${NC} %s\n" "$1"; }
error() { printf "${RED}[VibeOS]${NC} %s\n" "$1" >&2; }
ok()    { printf "${GREEN}[VibeOS]${NC} %s\n" "$1"; }
...
if [[ ! -d "$PLUGIN_DIR/scripts" ]] && [[ ! -d "$PLUGIN_DIR/.claude-plugin" ]]; then
  error "Directory does not look like a VibeOS plugin: $PLUGIN_DIR"
...
VIBEOS_STATE_DIR="$PROJECT_DIR/.vibeos"

# --- Step 3: Show what will be removed ----------------------------------------
echo ""
info "VibeOS Uninstaller"
echo "  =================================================="
echo ""
...
if [[ -d "$VIBEOS_STATE_DIR" ]]; then
  warn "Project state directory found at:"
  echo ""
  echo "    $VIBEOS_STATE_DIR"
  echo ""
...
# --- Step 6: Ask about .vibeos/ state directory --------------------------------
if [[ -d "$VIBEOS_STATE_DIR" ]]; then
  echo ""
  warn "The .vibeos/ directory contains project state:"
  ...
  echo ""
  printf "${BOLD}Remove .vibeos/ project state?${NC} [y/N]: "
  read -r confirm_state
  confirm_state="${confirm_state:-N}"

  # --- Step 7: Conditionally remove .vibeos/ ----------------------------------
  if [[ "$confirm_state" =~ ^[Yy]$ ]]; then
    rm -rf "$VIBEOS_STATE_DIR"
    ok "Removed project state directory: $VIBEOS_STATE_DIR"
  else
    info "Keeping project state directory: $VIBEOS_STATE_DIR"
  fi
fi

# --- Step 8: Success message --------------------------------------------------
echo ""
ok "VibeOS has been uninstalled."
