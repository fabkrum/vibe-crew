...
AUTO_FORMAT=$(jq -r '.formatting.auto_format // true' "$PROJECT_ROOT/.vibeos/config.json" 2>/dev/null || echo "true")
if [[ "$AUTO_FORMAT" == "false" ]]; then
  exit 0
fi

# Get configured formatter preference
FORMATTER=$(jq -r '.formatting.formatter // "prettier"' "$PROJECT_ROOT/.vibeos/config.json" 2>/dev/null || echo "prettier")
