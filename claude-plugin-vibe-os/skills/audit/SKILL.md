...
Run an OWASP Top 10 security audit on the current project. Scans for vulnerabilities across all ten OWASP categories, checks dependencies for known CVEs, and detects hardcoded secrets. Produces a structured audit report saved to `.vibeos/audits/`.

---

## Step 1: Pre-flight Check

Verify that VibeOS is initialized:

```bash
test -f ".vibeos/state.json" && echo "exists" || echo "missing"
```

If `.vibeos/state.json` does not exist, output EXACTLY this and stop:

```
VibeOS not initialized. Run /setup first.
```
...
3. Produce a structured findings JSON written to `.vibeos/audits/audit-<timestamp>.json`
...
Read the audit report from `.vibeos/audits/` and display findings grouped by severity.
...
Report:  .vibeos/audits/audit-<timestamp>.json
```
...
```bash
jq -r '.audit.auto_github_issues // false' .vibeos/config.json 2>/dev/null
```
...
```bash
ls -la .vibeos/audits/audit-*.json 2>/dev/null | tail -1
```

If the report file exists, confirm:

```
Full report saved to: .vibeos/audits/audit-<timestamp>.json
```
...
cat > ".vibeos/audits/audit-${AUDIT_TIMESTAMP}.json.tmp" << 'EOF'
...
- **Always** save the full report to `.vibeos/audits/` before exiting.
