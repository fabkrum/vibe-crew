...
- **Tier 2 (Feature Development):** Reads architecture diagrams from `.vibecrew/architecture/` for implementation context, injects codebase analysis docs (from `/onboard`) to skip convention re-discovery, runs a Clarify sub-step to resolve spec ambiguities, produces structured task plans (Files/Action/Verify/Done), designs component specifications, implements features within TDR boundaries, creates feature branches, writes application code, updates `component-tree.mmd` as new components are added, and makes conventional commits with `Co-Authored-By` trailers.
- **Standalone Feature Execution Mode:** When spawned by `/run-backlog` as a fresh-session Agent, detects the `# Feature Execution Context` header and runs Plan→Test autonomously with a clean 200k context window. Does not run Review or Docs (orchestrator handles those).
...
| `templates/ui-gotchas.md` | Condensed non-obvious UI rules (~75 lines, replaces 20 individual pattern file reads) | Template |
