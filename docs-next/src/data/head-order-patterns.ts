...
export const headOrderPatterns: HeadOrderPattern[] = [
  ...
  },
  {
    ...
    category: 'hints',
    description: 'The <script type="speculationrules"> element is the modern replacement for <link rel="prerender">. It allows declarative or list-based rules for prefetching and prerendering future navigations. The browser can fully render a page in a hidden tab before the user clicks, making navigation appear instant. Supported in Chrome 109+ and Edge. Falls back gracefully (ignored by unsupporting browsers).',
    implementation: 'Place at the bottom of <head> (same tier as prefetch). Syntax: <script type="speculationrules">{"prefetch": [{"urls": ["/next-page"]}]}</script> or use document rules: {"prerender": [{"where": {"href_matches": "/product/*"}, "eagerness": "moderate"}]}. Eagerness levels: conservative (on click), moderate (on hover), eager (immediately). Start with prefetch before prerender. Add a Speculation-Rules HTTP header as an alternative to inline JSON.',
    impact: 'Prerendering makes page transitions feel instant (0ms perceived load time). Chrome reports that prerendered navigations achieve near-perfect Core Web Vitals scores. The cost is memory and CPU — each prerendered page is a full hidden tab. Use moderate eagerness for navigation links and conservative for lower-probability targets. Excessive prerendering on mobile wastes battery and data.',
